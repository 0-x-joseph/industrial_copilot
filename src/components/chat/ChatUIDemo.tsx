 ...
 */
export const ChatHeader: React.FC<{ onSidebarToggle?: () => void }> = ({ onSidebarToggle }) => {
  return (
    <header className="flex items-center justify-between px-4 py-2.5 bg-[#313647] text-[#FFF8D4] shadow-sm border-b border-[#435663]">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="hidden sm:flex items-center gap-2">
          <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#435663] hover:bg-[#A3B087] transition-all duration-200 group">
            <Icon name="agent-selector" size={16} color="inverse" />
            <span className="text-sm font-medium">Data Analyst</span>
            <Icon name="dropdown" size={16} color="inverse" className="group-hover:rotate-180 transition-transform duration-200" />
          </button>
        ...
        <div className="flex items-center gap-0.5">
          ...
          >
            <Icon name="settings" size={20} color="inverse" />
          </button>
          <button 
            className="p-1.5 rounded-md hover:bg-[#435663] transition-colors"
            aria-label="Share chat"
          >
            <Icon name="share" size={20} color="inverse" />
          </button>
        </div>
      </div>
    </header>
...
export const ChatSidebar: React.FC = () => {
  ...
  return (
    <aside className="w-64 bg-[#F9FAFB] border-r border-[#E5E7EB] p-4 h-full overflow-y-auto">
      ...
      <div className="mb-6">
        <button className="flex items-center gap-2 w-full px-4 py-3 text-left rounded-lg bg-[#A3B087] text-white hover:bg-[#8B9474] transition-colors">
    ...
    <div className="absolute bottom-full left-0 right-0 mb-2 bg-white border border-[#E5E7EB] rounded-lg shadow-lg overflow-hidden z-50">
      ...
      <div className="flex border-b border-[#E5E7EB] bg-[#F9FAFB]">
        {[
          ...
          >
            <Icon name={tab.icon as any} size={16} color={activeTab === tab.id ? 'inverse' : 'secondary'} />
            {tab.label}
          </button>
        ))}
      </div>
      ...
      <div className="p-4 max-h-64 overflow-y-auto">
        ...
        {activeTab === 'agents' && (
          <div className="space-y-2">
            ...
            {[
              { name: 'Data Analyst', active: true, desc: 'Specializes in data analysis and insights' },
              { name: 'Energy Expert', active: false, desc: 'Focuses on energy optimization and sustainability' },
              { name: 'Business Strategist', active: false, desc: 'Provides strategic business guidance' },
              { name: 'Technical Writer', active: false, desc: 'Creates documentation and technical content' },
            ].map((agent) => (
              <label key={agent.name} className={`flex items-center gap-3 p-2 rounded-md cursor-pointer transition-colors ${
                agent.active ? 'bg-[#A3B087] text-white' : 'hover:bg-[#F3F4F6]'
              }`}>
                <input type="radio" name="agent" defaultChecked={agent.active} className="text-[#A3B087] focus:ring-[#A3B087]" />
                <Icon name="agent-selector" size={16} color={agent.active ? 'inverse' : 'secondary'} />
                <div className="flex-1">
                  <div className="text-sm font-medium">{agent.name}</div>
                  <div className={`text-xs ${agent.active ? 'text-[#FFF8D4]' : 'text-[#9CA3AF]'}`}>{agent.desc}</div>
                </div>
              </label>
            ))}
          </div>
        ...
        {activeTab === 'tabs' && (
          <div className="space-y-2">
            ...
            {[
              { name: 'Energy Dashboard', active: true, desc: 'Energy consumption & optimization' },
              { name: 'Sales Performance', active: false, desc: 'Sales metrics and trends' },
              { name: 'Operations', active: false, desc: 'Operational efficiency metrics' },
              { name: 'Financial Summary', active: false, desc: 'Financial KPIs and reports' },
            ].map((tab) => (
              <label key={tab.name} className={`flex items-center gap-3 p-2 rounded-md cursor-pointer transition-colors ${
                tab.active ? 'bg-[#A3B087] text-white' : 'hover:bg-[#F3F4F6]'
              }`}>
                <input type="radio" name="dashboard-tab" defaultChecked={tab.active} className="text-[#A3B087] focus:ring-[#A3B087]" />
                <Icon name="dashboard-tab" size={16} color={tab.active ? 'inverse' : 'secondary'} />
                <div className="flex-1">
                  <div className="text-sm font-medium">{tab.name}</div>
                  <div className={`text-xs ${tab.active ? 'text-[#FFF8D4]' : 'text-[#9CA3AF]'}`}>{tab.desc}</div>
                </div>
              </label>
            ))}
          </div>
        ...
        {activeTab === 'workspace' && (
          <div className="space-y-2">
            ...
            {[
              { name: 'Main', active: true, desc: 'Primary workspace', files: 12 },
              { name: 'Q4 Planning', active: false, desc: 'Quarterly planning workspace', files: 8 },
              { name: 'Sales Analysis', active: false, desc: 'Sales team workspace', files: 15 },
              { name: 'Energy Optimization', active: false, desc: 'Energy efficiency projects', files: 6 },
            ].map((workspace) => (
              <label key={workspace.name} className={`flex items-center gap-3 p-2 rounded-md cursor-pointer transition-colors ${
                workspace.active ? 'bg-[#A3B087] text-white' : 'hover:bg-[#F3F4F6]'
              }`}>
                <input type="radio" name="workspace" defaultChecked={workspace.active} className="text-[#A3B087] focus:ring-[#A3B087]" />
                <Icon name="workspace" size={16} color={workspace.active ? 'inverse' : 'secondary'} />
                <div className="flex-1">
                  <div className="text-sm font-medium">{workspace.name}</div>
                  <div className={`text-xs ${workspace.active ? 'text-[#FFF8D4]' : 'text-[#9CA3AF]'}`}>
